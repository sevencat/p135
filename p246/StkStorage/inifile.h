...
class CIniFileA
{
	...
	public:
		...
		bool Load( const std::string& fileName , bool bMerge = false);//默认包含海通期货

		// Load data from an input stream
		//void Load( std::istream& input , bool bMerge = false );
		void Load( std::istream& input , bool bMerge = false);//默认包含海通期货
	...
	public:
		...
		std::string ReadString( std::string sSection, std::string sKey ) const
		{
		...
		int ReadInteger(const std::string &sSection,const std::string &sKey,int defaultValue=0)
		{
			std::string retvalue=GetKeyValue(sSection,sKey);
			if(retvalue.empty())
				return defaultValue;
